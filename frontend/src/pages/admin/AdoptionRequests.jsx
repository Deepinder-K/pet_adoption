...
const AdoptionRequests = () => {
    ...
    async function ApproveRejectAdoptionRequest(id, status) {
        if (confirm('Are you sure')) {
            let url = `http://localhost:5000/approve-reject-adoption-request/${id}/${status}`;
            let response = await fetch(url);
            response = await response.json();
            // console.log(response);
    ...
    return (
        <>
            <BreadCrumbAdmin title="Pending Adoption"/>

            <div className="page container">
                {records && records.length > 0 && <h6 className="badge badge-secondary mb-4 fs-6">Adoption Request(s) {records.length}</h6>}

                {records && records.length > 0 ? (
                    records.map((x) => (
                        <div key={x.id} className="row border border-1 rounded shadow p-2 mb-5">
                                ...
                                {x.adoption_status === "Pending" && (
                                    <div>
                                        <button type="button" className="btn-danger px-2 rounded"
                                                onClick={() => ApproveRejectAdoptionRequest(x.id, "Rejected")}>
                                            Reject Request <i className="fa fa-times"></i>
                                        </button>

                                        <button type="button" className="btn-success px-2 ms-3 rounded"
                                                onClick={() => ApproveRejectAdoptionRequest(x.id, "Approved")}>
                            ...
                            <div className="col-md-12">
                                <div className="row">
                                    ...
                                    <div className="col-md-3">
                                        ...
                                        <p className="mb-0">Children friendly: {x.children_friendly}</p>
                                    </div>
